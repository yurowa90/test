...
import { BEHAVIOR_DOMAINS } from "../types.ts";
import { materialIssues, materialSignature, checkFigureSource } from "./integrity.ts";
import type { BehaviorDomain, TeacherInput } from "../types.ts";
...
export async function readRevisionMaterial(state: RevisionState, attachments: { name?: string; mimeType: string; data: string }[], apiKey: string, model: string, signal?: AbortSignal): Promise<Reading> {
  return callGemini<Reading>({ apiKey, model, signal, attachments, schema: READING_SCHEMA, temperature: 0.1,
...
export async function proposeRevision(state: RevisionState, input: TeacherInput, apiKey: string, model: string, signal?: AbortSignal): Promise<RevisionProposal> {
  if (!state.reading || !state.originalConfirmed || !state.targets.length) throw new GeminiError("원문 확인과 수정할 부분 선택을 먼저 마쳐 주세요.");
  const raw = await callGemini<RevisionProposal>({ apiKey, model, signal, schema: REVISION_SCHEMA, temperature: 0.3,
...
export function revisionToWorkspace(state: RevisionState, base: TeacherInput): Workspace {
  ...
  if (state.sourceMode === "reference") {
    const issues = revisionSourceIssues(state);
  ...
  const draft = createBankDraft(bank); draft.complexityConfirmed = false; draft.pickIds = bank.propositions.map(prop => prop.id);
  return { ...startWorkspace(input), step: "bank", bank, bankDraft: draft, analysis: { contentElements: [p.assessmentElement], assessmentElement: p.assessmentElement, assessmentGoal: p.assessmentGoal, behaviorDomain: state.targetBehavior, behaviorRationale: state.decisionReason, evidenceGoal: state.evidenceGoal, scenarios: [{ title: "기존 자료·문항 개선", stimulusType: c.figure ? "그림" : "제시문", description: c.body, cues: c.conditions.length ? c.conditions : ["제시 자료의 조건 대조"], inquiryContext: input.options.inquiryContext, sourcePlan: state.location || "아이디어 기반 합성 자료" }] }, reflection: { problem: state.teacherDiagnosis ?? "", reason: state.decisionReason, transfer: state.transfer ?? "" }, revisionRecord: revisionReport(state) };
}
...
export function revisionReport(state: RevisionState): string {
  ...
  return `# 문항 개선 기록\n\n출처·원문 위치: ${state.location || "미기록"}\n과목: ${state.subject}\n성취기준: ${state.standard}\n수정 요청: ${state.request}\n\n## 원문 확인본\n\n${state.reading ? content(state.reading.content) : state.text}\n\n${state.reading?.notes.join("\n") || ""}\n\n## 수정안\n\n${state.proposal ? content(state.proposal.content) : "미생성"}\n\n## AI 제안 이유와 전이 원리\n\n${state.proposal?.changes.map(ch => `- ${REVISION_PARTS[ch.part]}: ${ch.reason}\n  - 다른 문항에 적용할 원리: ${ch.principle}`).join("\n") || ""}\n\n## 재검토할 점\n\n${state.proposal?.warnings.join("\n") || ""}\n\n## 교사의 판단\n\n진단: ${state.teacherDiagnosis || "미기록"}\n채택: ${state.decision || "미기록"}\n근거: ${state.decisionReason || "미기록"}\n전이: ${state.transfer || "미기록"}`;
}

export function revisionSourceSignature(state: RevisionState): string {
  const c = state.proposal?.content;
  return c ? materialSignature(c.body, c.conditions, c.figure, JSON.stringify([state.reading, state.location])) : "";
}
...
export function changeRevision(state: RevisionState, patch: Partial<RevisionState>): RevisionState {
  ...
  if (!Object.entries(patch).some(([k, v]) => JSON.stringify(state[k as keyof RevisionState]) !== JSON.stringify(v))) return state;
  return { ...state, ...patch, history: state.reading || state.proposal ? [{ at: new Date().toISOString(), snapshot }, ...history].slice(0, 20) : history };
}
